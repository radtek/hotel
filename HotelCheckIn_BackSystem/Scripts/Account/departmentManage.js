...
function saveData() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            switch (value) {
                case "true":
                    parent.messager("提示", "保存成功！");
                    //$.messager.show({ title: '提示', msg: '保存成功！' });
                    break;
                case "false":
                    parent.messager("提示", "保存失败！");
                    //$.messager.show({ title: '提示', msg: '保存失败！' });
                    break;
                case "exist":
                    parent.messager("提示", "该部门已经存在！");
            ...
            if (r) {
                $.post(myurl, mydata,
                    function (data, status) {
                        if (data == "true") {
                            parent.messager("提示", "删除成功！");
                            //$.messager.show({ title: '提示', msg: '删除成功！' });
                            $('#dg').datagrid('reload');
                            refresh("refresh", false, id);
                        } else if (data == "exist") {
                            parent.messager("提示", "该部门底下有人员，不能删除！");
                            //$.messager.show({ title: '提示', msg: '该部门底下有人员，不能删除！' });
                            //$('#dg').datagrid('reload');
                            //refresh("refresh", false, id);
                        } else if (data == "exist2") {
                            parent.messager("提示", "该部门底下有部门，不能删除！");
                            //$.messager.show({ title: '提示', msg: '该部门底下有人员，不能删除！' });
                            //$('#dg').datagrid('reload');
                            //refresh("refresh", false, id);
                        } else if (data == "2") {
                            setTimeout(function () { location.href = "../../Login.aspx"; }, 4000);
                            parent.messager("提示", value.msg + "--登录过期，即将跳转到登陆页。");
                        }
                        else {
