...
var settingMenu = {
    async: {
        enable: true,
        url: "/DataService/WebService/ZtreeRole/ZtreeRoleService.ashx",
        autoParam: ["id", "name=n"],
...
function save_click() {
    ...
    if (!selectedNode) {
        parent.messager("提示", "请先点击选择角色！");
        return false;
    }

    var checkedNode = getCheckNode();
    if (checkedNode.length <= 0) {
        parent.messager("提示", "请给角色分配权限！");
        return false;
    }
    ...
    myurl = "/DataService/WebService/Roles/RoleService.ashx";
...
function delRole_click() {
    ...
    if (selectNode == null) {
        parent.messager("提示", "请点击角色列表中的角色！");
        return;
    }
        ...
        success: function (responseText, statusText) {
            if (statusText == "success") {
                if (responseText == "2") {
                    ...
                    parent.messager("提示", "--登录过期，即将跳转到登陆页。");
                    return true;
                }
                parent.messager("提示", responseText);
            }
            else {
                parent.messager("提示", responseText);
            }
                    ...
                    success: function(data, status) {
                        if (status == "success") {
                            if (data == "2") {
                                ...
                                parent.messager("提示", "该角色不能删除。");
                                return true;
                            }
                            parent.messager("提示", data);
                            $.fn.zTree.init($("#treeRole"), settingRole); //加载树
                            settingMenu.async.otherParam.roleid = "";
                            $.fn.zTree.init($("#treeMenu"), settingMenu); //加载树
                            //getPersonJs();//获取角色列表
                        } else {
                            parent.messager("提示", status);
                        }
                    }
                });
