$(function () {
    ...
    $.ajax({
        ...
        success: function (html) {
            ...
            for (var i = 0; i < length; i++) {
                $("#s_machine").append("<option value=" + val.rows[i].MacId + " >" + val.rows[i].Name + "</option>");
            }
        }
    });
    ...
    $("#t_checkinorder").datagrid({
        ...
        singleSelect: true,
        columns: [[
                    { field: "OrderId", title: "订单ID", width: 80, align: "center" },
					{ field: "RoomNum", title: "房间号", width: 80, align: "center" },
					{ field: "RoomType", title: "房间类型", width: 80, align: "center" },
					{ field: "Building", title: "楼栋", width: 50, align: "center" },
					{ field: "RoomCode", title: "房价代码", width: 70, align: "center", hidden: true },
					{ field: "RoomRate", title: "房价", width: 60, align: "center" },
                    { field: "CheckinType", title: "入住类型", width: 80, align: "center" },
					{ field: "ViPcardNum", title: "会员卡号", width: 130, align: "center", hidden: true },
					{ field: "ViPcardType", title: "会员卡类型", width: 80, align: "center", hidden: true },
                    { field: "PeopleNum", title: "入住人数", width: 80, align: "center" },
					{ field: "CheckinTime", title: "入住日期", width: 110, align: "center",
					    formatter: function (value, src) {
					        return eval("new " + value.split('/')[1]).Format("yyyy-MM-dd HH:mm");
					    }
					},
					{ field: "Hours", title: "钟点数", width: 50, align: "center", hidden: true },
					{ field: "AdvancePayment", title: "押金", width: 80, align: "center" },
					{ field: "AdvanceType", title: "预付方式", width: 80, align: "center", hidden: true },
					{ field: "CheckinImage", title: "入住签名", width: 80, align: "center",
					    formatter: function (value, src) {
					        return "<a href='javascript:void(0)' onclick='showCheckinImg(\"" + src.Orderid + "\")'>查看</a>";
					    },hidden:true
					},
                    { field: "MacName", title: "终端名称", width: 80, align: "center"},
					{ field: "OrderTime", title: "处理时间", width: 100, align: "center",
					    formatter: function (value, src) {
					        return eval("new " + value.split('/')[1]).Format("yyyy-MM-dd HH:mm");
					    }
					}
        ...
        onExpandRow: function (index, row) {
            $("#ddv-" + index).datagrid({
                ...
                height: "auto",
                columns: [[
                            { field: "Id", title: "姓名", width: 70,hidden:true},
