...
namespace HotelCheckIn_PlatformSystem.DataService.Dal
{
    public class ResMenuDal : BaseDal<ResMenu>
    {
        ...
        {
            Log.Debug("exist方法");
            var sql = "select * from t_ResMenu t where ResId='" + bean.ResId + "'";
        ...
        public override void Add(ResMenu bean)
        {
            ...
            var sql = "insert into t_ResMenu(" + sql1 + ") values(" + sql2 + ")";
        ...
        {
            Log.Debug("del方法参数：");
            var sql = "delete from t_ResMenu where ResId='" + bean.ResId + "'";
            mso.Execute(sql);
        }
        ...
        public override void Modify(ResMenu bean)
        {
            ...
            sql.Append("update t_ResMenu set");
            var i = 0;
        ...
        public override DataTable Query(ResMenu bean)
        {
            ...
            var sql = new StringBuilder();
            sql.Append("select * from t_ResMenu where 1=1 ");
            var list = new List<object>();
            var i = -1;
            if (!string.IsNullOrEmpty(bean.ResId))
            {
        ...
        public DataTable FindBy()
        {
            ...
            var sql = new StringBuilder();
            sql.Append("select * from t_ResMenu where parentid is null order by sortno");
