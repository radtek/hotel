...
{
    public class MachineDal : BaseDal<Machine>
    {
        public override bool Exist(Machine bean)
        {
            Log.Debug("Query方法参数：");
        ...
        public bool Exist(Machine bean, ref string url)
        {
            Log.Debug("Query方法参数：");
            var sql = new StringBuilder();
            var list = new List<string>();
        ...
        public override void Add(Machine bean)
        {
        ...
        public override void Del(Machine bean)
        {
        ...
        public override void Modify(Machine bean)
        {
        ...
        public string Modify(Machine bean, ref object[] objects)
        {
            Log.Debug("Modify方法参数：" + bean);
            var sql = new StringBuilder();
            sql.Append("update t_Machine set");
        ...
        public override DataTable Query(Machine bean)
        {
            Log.Debug("Query方法参数：");
            var sql = new StringBuilder();
        ...
        public override DataTable QueryByPage(Machine bean, int page, int rows)
        {
            throw new NotImplementedException();
        }

        public override DataTable QueryByPage(Machine bean,int index, int datacount, ref int sum)
        {
            throw new NotImplementedException();
        }
        ...
        {
            var dt = Query(new Machine() { JqId = jqid });
            if (dt.Rows.Count <= 0)
            {
                return false;
            }
            DateTime xtsj = Convert.ToDateTime(dt.Rows[0]["HeartbeatDt"]);
        ...
        /// <returns></returns>
        public DataTable FindByMain(Machine bean, int index, int datacount, ref int sum)
        {
            Log.Debug("Modify方法参数：" + bean);
            var sqlCount = new StringBuilder();
            var sqlDt = new StringBuilder();
            sqlCount.Append("select count(*) count from t_Machine tm where 1=1 and tm.Isdisabled=1 ");
            ...
            sqlDt.Append("from t_Machine tm where 1=1 and tm.Isdisabled=1 ");
