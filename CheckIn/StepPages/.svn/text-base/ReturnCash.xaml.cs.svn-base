...
{
    /// <summary>
    /// ReturnCash.xaml 的交互逻辑
    /// </summary>
    public partial class ReturnCash : Page
    {
        ...
        private void CcInit()
        {
            ...
            if (_bdBridge.IStatus == 9)
            {
                ...
                MessageBox.Show("出钞模块故障：请联系前台人员。点击“确定”按钮，返回主界面\n" + _bdBridge.ShowStatus(_bdBridge.IErrorCode, _bdBridge.IStatus),
                                         "错误");
                //此处需要注意，需要退卡
                labelMsg.Content = "退房失败，将在5秒后自动返回主界面";
                _timeout = _num + 5;
                ReturnCard();
                throw new BusinessException("出钞模块故障：请联系前台人员。点击“确定”按钮，返回主界面");
            }
